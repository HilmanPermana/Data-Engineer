...
SOURCE_TABLE_NAME = "cvm_taker_border_area_mtd_init"

TARGET_SCHEMA = "cvm"
TARGET_TABLE_NAME = "cvm_taker_border_area_mtd_init"
...
UPDATE_SET = ", ".join(
    [f"{col} = EXCLUDED.{col}" for col in COLUMNS if col != "strmonth"]
)
...
# Core ETL Functions
# ---------------------------------------------------------------------------
def fetch_source_data(conn, strmonth: str) -> list[dict]:
    """
    Fetch all rows from source table matching the given strmonth.
    Returns a list of dicts (one per row).
    """
    query = f"""
        SELECT {COLUMNS_SQL}
        FROM   {SOURCE_TABLE}
        WHERE  strmonth = %(strmonth)s
        ORDER  BY strmonth
    """
    logger.info(f"Fetching source data for strmonth = {strmonth} ...")
    with conn.cursor(cursor_factory=psycopg2.extras.RealDictCursor) as cur:
        cur.execute(query, {"strmonth": strmonth})
        rows = cur.fetchall()
    logger.info(f"  → {len(rows):,} rows fetched from source.")
    return [dict(row) for row in rows]


def delete_target_by_date(conn, strmonth: str) -> int:
    """
    Delete existing rows in the target table for the given strmonth
...
def sync_by_strmonth(strmonth: str, strategy: str = "upsert") -> None:
    logger.info("=" * 60)
    logger.info(f"START SYNC  |  strmonth={strmonth}  |  strategy={strategy}")
    ...
    try:
        ...
        # 1. Fetch
        rows = fetch_source_data(src_conn, strmonth)
        if not rows:
            logger.warning(f"No data found in source for strmonth={strmonth}. Exiting.")
            return

        # 2. Write to target
        if strategy == "replace":
            delete_target_by_date(tgt_conn, strmonth)
            query = f"""
                INSERT INTO {TARGET_TABLE} ({COLUMNS_SQL})
                VALUES ({PLACEHOLDERS})
            """
        ...
        tgt_conn.commit()
        logger.info(f"SYNC COMPLETE  |  {inserted:,} rows synced for strmonth={strmonth}")
...
def get_available_strmonths(source: bool = True) -> list[str]:
    """
    List all distinct strmonths available in the source (or target) table.
    ...
    try:
        with conn.cursor() as cur:
            cur.execute(f"SELECT DISTINCT strmonth FROM {table} ORDER BY strmonth DESC")
...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Sync CVM campaign summary data from Database A to B by strmonth."
    )
    parser.add_argument(
        "--date",
        type=str,
        default=str(date.today()),
        help="strmonth to sync (YYYY-MM-DD). Defaults to today.",
    )
    parser.add_argument(
        "--strategy",
    ...
    parser.add_argument(
        "--list-dates",
        action="store_true",
        help="List all available strmonths in the source table and exit.",
    )
...
if __name__ == "__main__":
    ...
    if args.list_dates:
        logger.info("Fetching available strmonths from source ...")
        dates = get_available_strmonths(source=True)
        print("\nAvailable strmonths in source:")
        for d in dates:
            print(f"  {d}")
        sys.exit(0)

    sync_by_strmonth(strmonth=args.date, strategy=args.strategy)