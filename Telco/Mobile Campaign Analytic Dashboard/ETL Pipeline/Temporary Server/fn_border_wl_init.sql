-- DROP FUNCTION cvm_data.fn_border_wl_init(date);

CREATE OR REPLACE FUNCTION cvm_data.fn_border_wl_init(p_strmonth date)
 RETURNS void
 LANGUAGE plpgsql
AS $function$
DECLARE
...
-- =========================
DELETE FROM cvm_data.cvm_wl_border_area_init WHERE strmonth = p_strmonth;
DELETE FROM cvm_data.cvm_wl_border_region_init WHERE strmonth = p_strmonth;
DELETE FROM cvm_data.cvm_wl_border_kabupaten_init WHERE strmonth = p_strmonth;
...
ins_region AS (
    INSERT INTO cvm_data.cvm_wl_border_region_init
    SELECT
        t.strmonth,
        t.border,
        t.tower,
...
ins_area AS (
    INSERT INTO cvm_data.cvm_wl_border_area_init
    SELECT
        t.strmonth,
        t.border,
        t.tower,
...
ins_kabupaten AS (
    INSERT INTO cvm_data.cvm_wl_border_kabupaten_init
    SELECT
        t.strmonth,
        t.border,
        t.tower,
